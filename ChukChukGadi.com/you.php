...
<body style="background-size: auto auto ">
	 ...
	 <form method="post" name="f1"  action="confirm.php">
 	<table border=0>
	<?php
		...
		$data=mysql_query("select * from trains where Train_name='Karnavati Express' ")or die(mysql_error());
			...
			if($not<=4)
			{
				...
				for ($in = 1; $in <= $not; $in++)
				{
					...
					if($in==$not)
					{
						print"<tr><td><p><button id='registerNew' type='submit'>Submit</button></p></td></tr>";
					}
				}
				}
				else{
					print"<script>alert('Sorry tickets not available for this train');</script>";
				}
